...
| REST historical endpoint | `GET /energy/historical?start=...&end=...` returns usage for period | Reads `hourly_usage` by date range and returns the aggregated totals; `start`/`end` bind to `LocalDateTime`, so an invalid date returns 400 | Implemented | A reversed range (`start` after `end`) is rejected with `400 Bad Request`. |
...
| GUI clean code | View/controller/client/DTO separation | Layout lives in `energy-view.fxml` (loaded via `FXMLLoader`); HTTP/Jackson are in `EnergyApiClient`; the controller only handles `@FXML` actions and label updates, displaying the totals the REST API aggregates | Implemented | Keep UI class from accumulating business or HTTP logic. |
